...
h="$(fresh_home)"
out="$(run_ss_codex "$h" "PLUGIN_ROOT=/x/.codex/plugins/cache/medley/medley/0.8.6-dev.1")"
...
h="$(fresh_home)"
out="$(run_ss_codex "$h" "CLAUDE_PLUGIN_DATA=$h/.codex/plugins/data/medley-medley")"
...
h="$(fresh_home)"
run_ss_codex "$h" "PLUGIN_ROOT=/x/.codex/plugins/cache/medley/medley/0.8.6-dev.1" >/dev/null
...
codex_env="PLUGIN_ROOT=/x/.codex/plugins/cache/medley/medley/0.8.6-dev.1"

# 11. Mission live + NO stop trust entry → warn.
h="$(fresh_home)"; mkdir -p "$h/.codex"
printf '%s\n' '[hooks.state."medley@medley:hooks/hooks.json:session_start:0:0"]' > "$h/.codex/config.toml"
...
h="$(fresh_home)"; mkdir -p "$h/.codex"
printf '%s\n' '[hooks.state."medley@medley:hooks/hooks.json:stop:0:0"]' > "$h/.codex/config.toml"
out="$(run_ss_codex_engine "$h" "$MISSION_ENGINE" "$codex_env")"
