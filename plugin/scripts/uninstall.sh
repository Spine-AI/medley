...
# This is the STABLE-CHANNEL uninstaller, so every CHANNEL-SCOPED path below carries the stable
# marketplace name. Both hosts derive their dirs as <plugin>-<marketplace> / <marketplace>, so one
# constant drives them all. Names are enumerated EXPLICITLY — a bare `medley*` glob would eat a
# co-installed dev-channel install (`medley-dev`) or an unrelated plugin, and the two channels are
# meant to be independently removable.
#
