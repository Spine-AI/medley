...
# This is the STABLE-CHANNEL uninstaller, so every path below carries the stable marketplace name.
# Both hosts derive their dirs as <plugin>-<marketplace> / <marketplace>, so one constant drives all
# five. Names are enumerated EXPLICITLY — a bare `medley*` glob would eat a co-installed dev-channel
# install (`medley-dev`) or an unrelated plugin, and the two channels are meant to be independently
# removable.
MARKET="medley"
...
# No trailing slash: the dev channel's dir is `medley-medley-dev/`, which a
# 'plugins/data/medley-medley/' pattern silently misses. Matches both hosts and both channels — this
# only kills running engine processes, so sweeping wider than MARKET is deliberate here.
