...
# A Codex config holding ONLY dev-channel tables. The STABLE uninstaller must not claim it.
cat > "$HOME_DIR/.codex/config.toml" <<'TOML'
[projects."/some/repo"]
trust_level = "trusted"

[hooks.state."medley@medley-dev:hooks/hooks.json:stop:0:0"]
trusted_hash = "abc"

[marketplaces.medley-dev]
source = "https://github.com/Spine-AI/medley-dev.git"
...
refute "$out" "medley entries in $HOME_DIR/.codex/config.toml" "stable does not claim dev-only Codex tables"
